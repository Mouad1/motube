    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Performance de la chaîne Instructify</p>
        </div>
        <div className="flex items-center gap-3">
            ...
            {PERIODS.map((opt) => (
              <button key={opt.value} onClick={() => setDays(opt.value)}
                className={`px-3 py-1.5 rounded-md text-sm transition-colors ${
                  days === opt.value ? "bg-teal-500 text-white font-medium" : "text-muted-foreground hover:text-foreground"
                }`}>
                {opt.label}
              </button>
            ))}
          ...
          <button onClick={handleSync} disabled={syncing}
            className="flex items-center gap-2 px-4 py-2 rounded-lg border border-border bg-card text-sm text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50">
      ...
      {isLoading ? (
        ...
        <>
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            <h2 className="text-sm font-medium text-muted-foreground mb-4">Vues par jour</h2>
            {snapshots.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-muted-foreground text-sm gap-2">
                <PlayCircle size={32} className="opacity-30" />
                <p>Aucune donnée — connecte YouTube et lance une sync</p>
                <a href="/api/auth/youtube" className="text-teal-600 hover:underline text-xs">
                  Connecter YouTube →
                </a>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={snapshots}>
                  ...
                  <XAxis dataKey="date" tick={{ fill: "#62625b", fontSize: 11 }} />
                  <YAxis tick={{ fill: "#62625b", fontSize: 11 }} />
                  <Tooltip contentStyle={{ background: "#ffffff", border: "1px solid #e0e0d9", borderRadius: 8 }}
                    labelStyle={{ color: "#211922" }} itemStyle={{ color: "#0d9488" }} />
          ...
          <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
            <h2 className="text-sm font-medium text-muted-foreground mb-4">Revenue estimé (USD/jour)</h2>
            {snapshots.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={snapshots}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" />
                  <XAxis dataKey="date" tick={{ fill: "#62625b", fontSize: 11 }} />
                  <YAxis tick={{ fill: "#62625b", fontSize: 11 }} tickFormatter={(v) => `$${v}`} />
                  <Tooltip contentStyle={{ background: "#ffffff", border: "1px solid #e0e0d9", borderRadius: 8 }}
          ...
          {(data?.platformStats?.length ?? 0) > 0 && (
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
              <h2 className="text-sm font-medium text-muted-foreground mb-4">Publications par plateforme</h2>
              <div className="flex flex-wrap gap-4">
                {data!.platformStats.map((p) => (
                  <div key={p.platform} className="flex items-center gap-3 bg-muted rounded-lg px-4 py-3">
                    <div className="w-2 h-2 rounded-full bg-teal-500" />
                    <span className="text-foreground font-medium capitalize">{p.platform}</span>
                    <span className="text-muted-foreground text-sm">{p.published}/{p.count} publiées</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {(data?.topVideos?.length ?? 0) > 0 && (
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
              <h2 className="text-sm font-medium text-muted-foreground mb-4">Top vidéos</h2>
              <div className="space-y-2">
                {data!.topVideos.map((v) => (
                  <div key={`${v.id}-${v.platform}`}
                    className="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="text-xs bg-muted text-muted-foreground rounded px-2 py-0.5 shrink-0 capitalize">{v.template}</span>
                      {v.url ? (
                    ...
                    <div className="flex items-center gap-4 shrink-0 ml-4">
                      <span className="text-sm text-muted-foreground">{fmtNum(v.views)} vues</span>
                      <span className="text-sm text-green-600">${v.revenue.toFixed(2)}</span>
                    </div>
                  </div>
                ))}
              </div>
